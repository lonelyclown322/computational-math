...
def newton_const_h(x, y, point):
    ...
    for i in range(len(finite_differences)):
        print(finite_differences[i])
    if point <= x[n // 2]:  # if point is closer to beginning we use FIRST Formula (forward)
        ...
        print("КОНЕЧНЫЕ РАЗНОСТИ ИСПОЛЬЗУЮЩИЕСЯ В ФОРМУЛЕ")
        for i in range(1, n):
            print(f"Δ^{i}y0 = {finite_differences[x0][i]}")
        ...
        print("КОНЕЧНЫЕ РАЗНОСТИ ИСПОЛЬЗУЮЩИЕСЯ В ФОРМУЛЕ")
        for i in range(1, n):
            print(f"Δ^{i}y{n - i - 1} = {finite_differences[n - i - 1][i]}")
