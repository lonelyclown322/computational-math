...
def plot_both_results(x, y, point, lagrange, newton):
    ...
    plt.scatter([point], [lagrange], marker="o",
                label=f"lagrange interpolation result ({round(lagrange, 5)})")  # marker circle
    plt.scatter([point], [newton], marker="o",
                label=f"newton interpolation result ({round(newton, 5)})")  # marker circle
    plt.plot(x, y, marker=".", label="input data")  # marker point
    plt.legend()
    plt.show()
